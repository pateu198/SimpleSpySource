...
local Highlight = loadstring(game:HttpGet("https://github.com/exxtremestuffs/SimpleSpySource/raw/2.3-beta/highlight.lua"))()
local Serializer = loadstring(game:HttpGet("https://github.com/exxtremestuffs/SimpleSpySource/raw/2.3-beta/serializer.lua"))()
...
function newRemote(type, name, args, remote, function_info, blocked, src)
    ...
    remoteFrame.Text.Text = Serializer.types.string(name:sub(1, 25), { stringNoQuotes = true })
...
function genScript(remote, args)
    local genScript = "-- Generated by SimpleSpy Remote Spy v2.3 | @exx#9394\n"
    local hasArgs
    if #args > 0 then
        genScript = genScript .. Serializer:serialize(args, "args") .. "\n"
        hasArgs = true
    end
    return genScript .. string.format("%sremote:%s(%s)", Serializer:serialize(remote, "remote"), remote:IsA("RemoteEvent") and "FireServer" or "InvokeServer", hasArgs and "args" or "")
    -- prevTables = {}
    -- local gen = ""
    -- if #args > 0 then
